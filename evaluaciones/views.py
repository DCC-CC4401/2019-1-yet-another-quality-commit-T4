...
def evaluacion_evaluador(request, evaluacion_id, equipo_id, estudiante_id):
    print(evaluacion_id, equipo_id, estudiante_id)
    evaluacion = Evaluacion.objects.get(id=evaluacion_id)
    r = evaluacion.rubrica
    estudiante = Estudiante.objects.get(id=estudiante_id)
    ...
    return render(request, 'Eval_interface/evaluacion_evaluador.html',
                  context={'data' : data,
                           'header' : header,
                           'grupo' : equipo,
                           'estudiante' : estudiante,
...
def postevresult_evaluador(request):
    p = request.POST
...
def evev(request, evaluacion_id):
    print(evaluacion_id)
    eval = Evaluacion.objects.get(id=evaluacion_id)
    return render(
        request,
        'Eval_interface/ficha_evaluacion_evaluador.html',
        context={
            "evaluacion": eval,
            "equipos": eval.curso.equipos.all(),
            "integrantes": {
